...
def create_blockers_sheet(wb):
    ...
    data = [
        [
            ...
            "P0",
            "CLOSED",
            "scripts/init_models.sh generates model_config.json.",
            "MVP_Cloud",
        ],
        ...
        [
            ...
            "P0",
            "CLOSED",
            "Ceph S3 Buckets created via setup_tenants.sh.",
            "MVP_Cloud",
        ],
        ...
        [
            "BLK-034",
            "Phantom DB Dependency",
            "P0",
            "CLOSED",
            "Removed asyncpg; Switched to etcd3 for State.",
            "MVP_Cloud",
        ],
        ...
        [
            ...
            "P0",
            "CLOSED",
            "Added Docker Volumes for Ceph (Data+Config) & Etcd.",
            "MVP_Cloud",
        ],
        [
            "BLK-037",
            ...
            "P0",
            "CLOSED",
            "Added scripts/fix_perms.sh to pre-boot sequence.",
            "MVP_Cloud",
        ],
...
def create_actions_sheet(wb):
    ...
    data = [
        [
            "ACT-100",
            "Create scripts/check_avx512.sh",
            "Platform",
            "None",
            "T-0",
            "Script returns 0 on AVX-512 hosts; warns on others.",
        ],
        [
            "ACT-101",
            ...
            "None",
            "T-0",
            "a2a-python>=0.3, python-etcd3, aioboto3 (Async S3).",
        ],
        [
            "ACT-102",
            ...
            "ACT-101",
            "T-0",
            "Engine uses etcd3 for locking; aioboto3 for S3.",
        ],
        [
            "ACT-103",
            "Refactor workflows/main.py",
            "Protocol",
            "ACT-101",
            "T-0",
            "Endpoints replaced with A2AFastAPI router.",
        ],
        [
            "ACT-104",
            ...
            "None",
            "T-0",
            "Map Ceph (Data+Conf) / Etcd Volumes; Set Vault Root Token; Healthchecks added; restart: unless-stopped.",
        ],
        [
            "ACT-105",
            "Create scripts/init_models.sh",
            "Platform",
            "ACT-104",
            "T-1",
            "TinyLlama GGUF + model_config.json generated.",
        ],
        [
            "ACT-106",
            "Create scripts/setup_tenants.sh",
            "Security",
            "ACT-104",
            "T-1",
            "User tenant-a created in Ceph RGW.",
        ],
        [
            "ACT-107",
            "Configure budget-proxy/routes.yaml",
            "Platform",
            "None",
            "T-1",
            "Route tinyllama to http://ovms:9001 (REST).",
        ],
        ["ACT-108", "Implement MCP Bridge", "Integration", "ACT-102", "T-2", "engine.py calls MCP tools dynamically."],
        [
            "ACT-109",
            "Create scripts/fix_perms.sh",
            "DevOps",
            "ACT-104",
            "T-0",
            "Script chowns ./data/ directories to container UIDs.",
        ],
        [
            "ACT-110",
            "Update Deployment_Order logic",
            "DevOps",
            "ACT-105",
            "T-0",
            "setup_tenants.sh waits for ceph healthcheck. engine.py waits for etcd healthcheck.",
        ],
        [
            "ACT-111",
            ...
            "ACT-101",
            "T-0",
            'CI/Script runs python -c "import etcd3; ..." to catch naming ambiguity.',
        ],
    ]
...
def create_code_gaps_sheet(wb):
    ...
    data = [
        [
            "CG-101",
            "requirements.txt",
            'Remove asyncpg. Add python-etcd3==0.12.0 (State), aioboto3==11.3.0 (S3), boto3, tenacity==8.2.3, grpcio==1.60.0 (Pinned). Smoke test: python -c "import etcd3; import aioboto3; import grpc".',
        ],
        [
            "CG-102",
            "src/orchestrator/engine.py",
            "Use etcd3 client for Distributed Locking. Use aioboto3 for Artifact persistence.",
        ],
        ["CG-103", "workflows/main.py", 'Remove @app.post("/task"). Add app.include_router(A2AFastAPI(agent).router).'],
        [
            "CG-104",
            "docker-compose.yml",
            "Ceph: Vols ./data/ceph:/var/lib/ceph, ./data/ceph_conf:/etc/ceph. Etcd: Vol ./data/etcd:/etcd-data, Env ETCD_DATA_DIR=/etcd-data, Command /usr/local/bin/etcd --data-dir=/etcd-data. OpenBao: Command server -dev -dev-root-token-id=root.",
        ],
        [
            "CG-105",
            "docker-compose.yml",
            "OVMS: Expose Port 9001 (REST). Env ONEDNN_MAX_CPU_ISA=AVX512_CORE. CMD: --config_path /models/model_config.json. Healthchecks on etcd (etcdctl endpoint health), ceph (curl -f http://localhost:8000), ovms (curl -f http://localhost:9001/v2/health/live). restart: unless-stopped for all.",
        ],
        ["CG-106", "config/budget-proxy.yaml", "Add routing rule: model: tinyllama -> target: http://ovms:9001."],
        [
            "CG-107",
            "scripts/check_avx512.sh",
            "Enforce: If AVX-512 not found, exit 1 (Fail Fast). User must override with FORCE_NO_AVX=1.",
        ],
        [
            "CG-108",
            "scripts/fix_perms.sh",
            "Create script to chown -R 167:167 ./data/ceph and chown -R 1001:1001 ./data/etcd before boot.",
        ],
    ]
...
def create_deployment_order_sheet(wb):
    ...
    data = [
        ...
        [
            ...
            "Wait for docker compose ps -> healthy",
        ],
...
def create_architecture_gaps_sheet(wb):
    ...
    data = [
        [
            "GAP-012",
            "Storage Isolation",
            "Logical: Ceph S3 Policy + Tenant Buckets",
            "Physical: Rook-Ceph Block Devices",
        ],
...
def create_validation_gates_sheet(wb):
    ...
    data = [
        ...
        [
            ...
            "Etcd Lock holds; Ceph Data/Keys persist; No Permission Denied errors.",
        ],
        [
            "VG-107",
            "Resilience (Recreate)",
            "docker compose down && docker compose up -d",
            "Data persists (Volume check); Identity stable.",
        ],
