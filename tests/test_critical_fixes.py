...
def test_workflow_manifest_has_single_templates_key():
    ...
    workflow_defs_path = os.path.join(os.path.dirname(__file__), "..", "workflows", "workflow_defs.py")

    with open(workflow_defs_path, encoding="utf-8") as f:
        lines = f.readlines()
    ...
    for i, line in enumerate(lines, 1):
        ...
        if in_spec:
            ...
            if ('"templates":' in line or "'templates':" in line) and ":" in line and "templates" in line:
    ...
    assert len(templates_keys) == 1, (
        f"Expected 1 'templates' key in spec, found {len(templates_keys)} at lines {templates_keys}"
    )
    ...
    assert '"onExit": "notify-agent"' in content or "'onExit': 'notify-agent'" in content, (
        "Missing onExit configuration"
    )
...
def test_trace_viewer_uses_parameterized_queries():
    ...
    trace_viewer_path = os.path.join(os.path.dirname(__file__), "..", "src", "trace-viewer", "trace_viewer.py")

    with open(trace_viewer_path, encoding="utf-8") as f:
        content = f.read()
...
def test_goose_client_no_unused_subprocess_import():
    ...
    with open(goose_path, encoding="utf-8") as f:
        lines = f.readlines()
    ...
    subprocess_imports = [
        line for line in import_lines if line == "import subprocess" or line.startswith("import subprocess,")
    ]
