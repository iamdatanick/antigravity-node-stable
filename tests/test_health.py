...
# ---------------------------------------------------------------------------
# Helper: mock httpx response and client
# ---------------------------------------------------------------------------
...
def _make_client(responses: dict[str, int] | None = None, fail_urls: set[str] | None = None):
    """Create a mock httpx.AsyncClient.

    Parameters
    ----------
    responses : dict mapping URL substrings to HTTP status codes.
    fail_urls : set of URL substrings that should raise ConnectError.
    """
...
class TestCheckUrl:
    ...
    async def test_healthy_200(self):
        ...
        client = _make_client({"example": 200})
        result = await _check_url(client, "svc", "http://example:8080/health")
    ...
    async def test_unhealthy_500(self):
        ...
        client = _make_client({"example": 500})
        result = await _check_url(client, "svc", "http://example:8080/health")
    ...
    async def test_accept_any_500_still_healthy(self):
        ...
        client = _make_client({"example": 500})
        result = await _check_url(client, "svc", "http://example:8080/health", accept_any=True)
    ...
    async def test_connection_error(self):
        ...
        client = _make_client(fail_urls={"example"})
        result = await _check_url(client, "svc", "http://example:8080/health")
    ...
    async def test_timeout_marks_unhealthy(self):
        ...
        client.get = AsyncMock(side_effect=_raise_timeout)

        result = await _check_url(client, "slow", "http://slow:8080/health")
