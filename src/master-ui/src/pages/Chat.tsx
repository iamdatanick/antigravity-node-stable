...
export default function Chat() {
  ...
  // Default to first available model, fallback to gpt-4o
  const defaultModel = modelsData?.data?.[0]?.id || "gpt-4o";
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        for await (const chunk of streamChat(chatMessages, defaultModel, abortRef.current.signal)) {
    ...
    [messages, addMessage, appendContent, updateMessage, setStreaming, defaultModel],
  );
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            Talk to Antigravity Node — model: {defaultModel}
          </p>
        </div>
        {messages.length > 0 && (
          <button
