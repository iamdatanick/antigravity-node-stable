...
from agentic_workflows.agents.base import (
    BaseAgent, AgentConfig, AgentContext, AgentResult, AgentState
)
from agentic_workflows.security import (
    PromptInjectionDefense, RateLimiter, KillSwitch, ScopeValidator, Scope
)
from agentic_workflows.orchestration import (
    Pipeline, CircuitBreaker, Retrier, Supervisor, ParallelExecutor
)
from agentic_workflows.observability import (
    MetricsCollector, AlertManager, AgentTracer
)
from agentic_workflows.context import (
    ContextGraph, Provenance
)
from agentic_workflows.handoffs import (
    HandoffManager, CheckpointManager, RecoveryOrchestrator
)
...
class PhucPipeline:
    ...
    async def execute(self, input_data: Union[str, Dict, PipelineInput]) -> PipelineResult:
    ...
    async def _execute_shipper(self, task: Dict) -> Dict:
        ...
        if self.mcp:
            try:
                await self.mcp.call_tool("workers_deploy", {
                    "name": self.name,
                    "code": task.get("code", "")[:10000]
                })
            except:
                pass
