...
class TelemetryCollector:
    ...
    def emit(
        self, event_type: EventType, data: dict[str, Any] = None, status: str = "ok"
    ) -> TelemetryEvent:
    ...
    def record_tokens(self, input_tokens: int, output_tokens: int, model: str = "unknown"):
    ...
    def check_budget(self, budget: float) -> bool:
        ...
        if self.total_cost > budget:
            self.emit(
                ...
                {"budget": budget, "current_cost": self.total_cost},
                status="error",
            )
            return False
    ...
    def get_summary(self) -> dict:
        ...
        tool_events = [
            ...
            if e.event_type in [EventType.TOOL_START, EventType.TOOL_COMPLETE]
        ]
    ...
    def _get_parent_span_id(self, span_id: str) -> str | None:
    ...
    def _calculate_total_duration(self) -> float | None:
