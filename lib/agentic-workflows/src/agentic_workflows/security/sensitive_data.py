...
class SensitiveDataFilter:
    ...
    def redact(
        self, text: str, min_severity: Severity = Severity.LOW
    ...
    def _compile_patterns(self) -> list[tuple[str, re.Pattern, DataCategory, Severity, str]]:
        ...
        patterns = [
            # PII - Critical
            ("ssn", r"\b\d{3}-\d{2}-\d{4}\b", DataCategory.PII, Severity.CRITICAL, "full"),
            (
                "ssn_no_dash",
            ...
            # PHI - Critical
            ("mrn", r"\bMRN[:\s#]*\d{6,12}\b", DataCategory.PHI, Severity.CRITICAL, "full"),
            (
                "patient_id",
                r"\b(?:patient|pt)[:\s#-]*(?:id)?[:\s#-]*\d{5,10}\b",
                DataCategory.PHI,
                Severity.CRITICAL,
                "full",
            ),
            (
                "diagnosis_icd",
                r"\b(?:ICD-?(?:10|11)|diagnosis)[:\s]*[A-Z]\d{2}(?:\.\d{1,4})?\b",
                DataCategory.PHI,
                Severity.HIGH,
                "full",
            ),
            ...
            (
                "aws_secret",
                r"(?:aws[_-]?secret|AWS_SECRET)[=:\s]+['\"]?([A-Za-z0-9/+=]{40})['\"]?",
                DataCategory.CREDENTIALS,
                Severity.CRITICAL,
                "full",
            ),
            ...
            (
                "internal_url",
                r"https?://[a-z0-9.-]+\.(?:internal|corp|local|lan|intra)\b[^\s]*",
                DataCategory.INTERNAL,
                Severity.MEDIUM,
                "domain_only",
            ),
...
def check_for_secrets(text: str) -> list[str]:
    ...
    secrets = [
        f"{m.data_type}: {m.redacted}" for m in matches if m.category == DataCategory.CREDENTIALS
    ]
