...
class PromptInjectionDefense:
    ...
    INJECTION_PATTERNS = [
        ...
        (
            r"(?:what|show|reveal|display|print|output)\s+(?:is\s+)?(?:your\s+)?(?:system\s+)?prompt",
            0.9,
        ),
    ...
    def __init__(
        ...
        self._compiled_injection = [
            (re.compile(p, re.IGNORECASE), w) for p, w in self.INJECTION_PATTERNS
        ]
        self._compiled_heuristic = [
            (re.compile(p, re.IGNORECASE), w) for p, w in self.HEURISTIC_PATTERNS
        ]

        if custom_patterns:
            self._compiled_injection.extend(
                [(re.compile(p, re.IGNORECASE), w) for p, w in custom_patterns]
            )
    ...
    def scan(self, text: str) -> ScanResult:
        ...
        if not text or not text.strip():
            return ScanResult(
                threat_level=ThreatLevel.NONE, confidence=1.0, details={"reason": "Empty input"}
            )
    ...
    def _analyze_content_structure(self, text: str) -> float:
        ...
        marker_count = sum(
            1 for pattern, flags in conversation_markers if re.search(pattern, text, flags)
        )
        if marker_count >= 2:
            score += 0.3
    ...
    def get_explanation(self, result: ScanResult) -> str:
        ...
        if result.details.get("indirect_matches"):
            ...
            explanations.append(f"Found {len(indirect)} indirect injection pattern(s)")

        if result.details.get("heuristic_matches"):
            heuristics = result.details["heuristic_matches"]
            explanations.append(f"Found {len(heuristics)} suspicious indicator(s)")

        if result.details.get("semantic_score", 0) > 0.3:
            explanations.append(
                f"Semantic analysis flagged content (score: {result.details['semantic_score']:.2f})"
            )
        ...
        return (
            ...
            f"(confidence: {result.confidence:.0%}). " + " ".join(explanations)
        )
