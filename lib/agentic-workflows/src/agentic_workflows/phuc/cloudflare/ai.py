...
class WorkersAI:
    ...
    async def chat(
        ...
        response = await client.post(f"{self.base_url}/{model.value}", json=payload)
    ...
    async def embed(
        self, texts: list[str], model: AIModel = AIModel.BGE_BASE
        ...
        for text, vec_data in zip(texts, vectors):
            embeddings.append(EmbeddingResult(text=text, vector=vec_data, dimensions=len(vec_data)))
    ...
    async def generate_campaign(
        self, therapeutic_area: str, objectives: list[str], context: str = ""
    ) -> dict:
...
Generate a detailed campaign strategy in JSON format."""
        ...
        response = await self.generate(
            prompt=prompt, system=system, model=AIModel.LLAMA_3_1_70B, temperature=0.7
        )
    ...
    async def answer_pharma_question(self, question: str, context: list[str]) -> str:
