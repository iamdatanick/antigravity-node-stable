...
class R2Storage:
    ...
    async def upload(
        ...
        response = await client.put(
            f"{self.base_url}/objects/{key}", content=content, headers=headers
        )
    ...
    async def get_metadata(self, key: str) -> R2Object | None:
        """Get object metadata."""
        client = await self._get_client()
    ...
    async def list_objects(
        self, prefix: str = "", limit: int = 1000, cursor: str = None
    ) -> tuple[list[R2Object], str | None]:
        ...
        next_cursor = data.get("result", {}).get("truncated") and data.get("result", {}).get(
            "cursor"
        )
