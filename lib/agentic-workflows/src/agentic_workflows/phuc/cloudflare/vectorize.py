...
class Vectorize:
    ...
    async def get_by_ids(self, ids: list[str]) -> list[VectorMatch]:
        ...
        response = await client.post(f"{self.base_url}/getByIds", json={"ids": ids})
        response.raise_for_status()
        ...
        for v in response.json().get("result", {}).get("vectors", []):
            matches.append(
                VectorMatch(
                    id=v["id"], score=1.0, values=v.get("values"), metadata=v.get("metadata", {})
                )
            )
    ...
    async def delete(self, ids: list[str]) -> dict:
        ...
        response = await client.post(f"{self.base_url}/deleteByIds", json={"ids": ids})
        response.raise_for_status()
        return response.json()
    ...
    async def search_documents(
        self, query_vector: list[float], user_id: str = None, doc_type: str = None, top_k: int = 5
    ) -> list[VectorMatch]:
    ...
    async def index_document_chunks(
        self, doc_id: str, chunks: list[dict], embeddings: list[list[float]]
    ) -> dict:
