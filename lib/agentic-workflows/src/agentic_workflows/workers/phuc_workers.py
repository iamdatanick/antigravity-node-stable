...
class CloudflareWorker(ABC):
    ...
    async def execute(self, tool: str, args: dict[str, Any] = None) -> WorkerResult:
        ...
        if self._rate_limiter and not self._rate_limiter.allow(self.name):
            return WorkerResult(
                worker=self.name, tool=tool, success=False, error="Rate limit exceeded"
            )

        # Check circuit breaker
        if self._circuit_breaker and not self._circuit_breaker.allow():
            return WorkerResult(
                worker=self.name, tool=tool, success=False, error="Circuit breaker open"
            )
...
class R2Worker(CloudflareWorker):
    ...
    skills: builtins.list[str] = field(default_factory=lambda: ["r2"])
    tools: builtins.list[str] = field(
        default_factory=lambda: [
            "r2_get",
            "r2_put",
            "r2_list",
    ...
    async def put(self, key: str, value: Any, content_type: str = None) -> WorkerResult:
        return await self.execute(
            "r2_put", {"key": key, "value": value, "content_type": content_type}
        )
...
class AIWorker(CloudflareWorker):
    ...
    async def generate(
        ...
        return await self.execute(
            "ai_generate", {"prompt": prompt, "model": model, "max_tokens": max_tokens}
        )
    ...
    async def vector_insert(
        self, id: str, vector: list[float], metadata: dict = None
    ) -> WorkerResult:
        return await self.execute(
            "vectorize_insert", {"id": id, "vector": vector, "metadata": metadata}
        )
...
class AnalyticsWorker(CloudflareWorker):
    ...
    async def track(self, event: str, properties: dict = None) -> WorkerResult:
        return await self.execute(
            "attribution_track", {"event": event, "properties": properties or {}}
        )

    async def query_attribution(self, start_date: str, end_date: str, **filters) -> WorkerResult:
        return await self.execute(
            "attribution_query", {"start_date": start_date, "end_date": end_date, **filters}
        )
    ...
    async def generate_report(self, report_type: str, params: dict = None) -> WorkerResult:
        return await self.execute("report_generate", {"type": report_type, "params": params or {}})
...
class SecurityWorker(CloudflareWorker):
    ...
    skills: list[str] = field(
        default_factory=lambda: ["injection-defense", "scope-validator", "pii-detector"]
    )
    tools: list[str] = field(
        default_factory=lambda: [
            # Injection defense
...
class CAMARAWorker(CloudflareWorker):
    ...
    async def check_sim_swap(
        self, phone_number: str, operator: str = "telefonica", max_age: int = 24
    ) -> WorkerResult:
        return await self.execute(
            "checkSimSwap", {"phoneNumber": phone_number, "operator": operator, "maxAge": max_age}
        )
    ...
    async def check_roaming(self, phone_number: str, operator: str = "telefonica") -> WorkerResult:
        return await self.execute(
            "checkRoamingStatus", {"phoneNumber": phone_number, "operator": operator}
        )
...
class WorkerPool:
    ...
    def get_worker(self, name: str) -> CloudflareWorker | None:
        """Get worker by name"""
        return self.workers.get(name)

    def get_worker_for_skill(self, skill_name: str) -> CloudflareWorker | None:
    ...
    def get_worker_for_tool(self, tool: str) -> CloudflareWorker | None:
    ...
    async def execute_parallel(self, calls: list[dict]) -> list[WorkerResult]:
        ...
        tasks = [self.execute(call["worker"], call["tool"], call.get("args", {})) for call in calls]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        return [
            r
            if isinstance(r, WorkerResult)
            else WorkerResult(worker="unknown", tool="unknown", success=False, error=str(r))
            for r in results
        ]
